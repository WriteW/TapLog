...
class DailyViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var selectedEntry: String? by mutableStateOf(null)
        private set

    // 页面切换发射
    val navigationEvent = MutableSharedFlow<Pair<String, String>>() // 类型和文本信息
    // 图片查看器状态
    var viewingImageEntry = mutableStateOf<DailyEntry?>(null)
        private set
    init {
        loadData()
    }
    ...
    fun navigateToPortal() {
        viewModelScope.launch {
            navigationEvent.emit(Pair("portal", ""))
        }
    }
    fun navigatePop() {
        viewModelScope.launch {
    ...
    private fun groupEntries(entries: List<DailyEntry>): List<TimelineGroup> {
        ...
        entries.forEach { entry ->
            if (currentBatch.isEmpty()) {
                ...
                val isPin = entry.isPin

                if (isWithin10Min && !isCurrentBig && !isLastBig && !isPin) {
                    currentBatch.add(entry)
                } else {
                    // 结算上一组
    ...
    fun toggleEntryPin(entryId: String) {
        viewModelScope.launch {
            ...
            if (entry != null) {
                ...
                android.util.Log.d("dog is cute", "entry:${newEntry.isPin}")
